...
INPUTOUTPUTCURVEFUNCTIONDATA_ONE_OF_SCHEMAS = [
    "LinearFunctionData",
    "PiecewiseLinearData",
...
class InputOutputCurveFunctionData(BaseModel):
    """
    InputOutputCurveFunctionData
    """

    # data type: QuadraticFunctionData
    oneof_schema_1_validator: Optional[QuadraticFunctionData] = None
    # data type: LinearFunctionData
    oneof_schema_2_validator: Optional[LinearFunctionData] = None
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = InputOutputCurveFunctionData.model_construct()
        error_messages = []
        match = 0
        ...
        if match > 1:
            # more than 1 match
            raise ValueError(
                "Multiple matches found when setting `actual_instance` in InputOutputCurveFunctionData with oneOf schemas: LinearFunctionData, PiecewiseLinearData, QuadraticFunctionData. Details: "
                + ", ".join(error_messages)
            )
        elif match == 0:
            # no match
            raise ValueError(
                "No match found when setting `actual_instance` in InputOutputCurveFunctionData with oneOf schemas: LinearFunctionData, PiecewiseLinearData, QuadraticFunctionData. Details: "
                + ", ".join(error_messages)
            )
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if match > 1:
            # more than 1 match
            raise ValueError(
                "Multiple matches found when deserializing the JSON string into InputOutputCurveFunctionData with oneOf schemas: LinearFunctionData, PiecewiseLinearData, QuadraticFunctionData. Details: "
                + ", ".join(error_messages)
            )
        elif match == 0:
            # no match
            raise ValueError(
                "No match found when deserializing the JSON string into InputOutputCurveFunctionData with oneOf schemas: LinearFunctionData, PiecewiseLinearData, QuadraticFunctionData. Details: "
                + ", ".join(error_messages)
            )
        else:
            return instance
