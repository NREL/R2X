...
def create_external_parser() -> PluginComponent:
    ...
    input_defaults.extend([DefaultFile.from_path("defaults/reeds_input_ext.json", module="r2x_mock_plugin")])
...
def create_external_exporter() -> PluginComponent:
    ...
    # REEDS is input-only
    return PluginComponent(
        config=TestExternalConfig, exporter=TestExternalExporter, export_defaults=export_defaults, fmap=fmap
    )


# R2X expects a function that returns Dict[str, PluginComponent]
def create_plugin_components() -> dict[str, PluginComponent]:
    components = {"ExternalParser": create_external_parser(), "ExternalExporter": create_external_exporter()}
